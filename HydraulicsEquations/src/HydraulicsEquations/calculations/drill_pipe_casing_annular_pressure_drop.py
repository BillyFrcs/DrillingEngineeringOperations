...
class DrillPipeCasingAnnularPressureDrop(object):
    ...
    def calculate_velocity(self):
        ...
        """
        self.velocity_ft = 24.48 * self.flow_rate / (math.pow(self.inside_diameter, 2) - math.pow(4.5, 2))
    ...
    def calculate_effective_viscosity(self):
        ...
        """
        self.effective_viscosity = (100 * self.ka * ((math.pow((2.4 * self.velocity_ft) / (self.inside_diameter - 4.5), 0.275 - 1))) * math.pow((2 * 0.275 + 1) / (3 * 0.275), 0.275)) 
    ...
    def calculate_reynolds_number(self):
        ...
        """
        self.reynolds_number = (15.467 * self.velocity_ft * (self.inside_diameter - 4.5) * self.mud_density) / self.effective_viscosity
    ...
    def calculate_friction_factor(self):
        ...
        fa = 24 / NRea
        """
        self.friction_factor = 24 / self.reynolds_number
    ...
    def calculate_pressure_loss(self):
        ...
        """
        self.pressure_loss_psi = ((self.friction_factor * math.pow(self.velocity_ft, 2) * self.mud_density) / (92916 * (self.inside_diameter - 4.5))) * self.length
    ...
    def display_results(self):
        print("\nDP – CASING ANNULAR PRESSURE DROP")
        print("-" * 30)
        print('Pipe "n" and "K" values:')
        print(f"na = {self.na}")
        print(f"Ka = {self.ka}")

        print('\nANNULAR INTERVAL #3 (9"5/8 CASING - 4"1/2 DRILL PIPE): Casing ID = 8.835"')
